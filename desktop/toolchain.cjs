...
function prepareBundledToolchain({ directory, nodePath, pnpmEntry, platform = process.platform }) {
  if (!directory || !nodePath || !pnpmEntry) throw new Error('Bundled toolchain requires directory, nodePath, and pnpmEntry.')
  ...
  fs.symlinkSync(nodePath, nodeShim)

  const writePnpmShim = (name, extra = '') => {
    const target = path.join(binDir, name)
    const body = `#!/bin/sh\nexec ${shellQuote(nodePath)} ${shellQuote(pnpmEntry)} ${extra}"$@"\n`
