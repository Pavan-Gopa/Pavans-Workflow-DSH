...
const required = [
  '.dsh/roles.example.yaml', '.dsh/roles.recommended.yaml', '.dsh/skills/pavan-workflow/SKILL.md', '.dsh/skills/ponytail/SKILL.md',
...
if (!repoOnly) {
  ...
  let dsh = spawnSync('dsh', ['--version'], { encoding: 'utf8' })
  if (dsh.status === 0) {
    ok(`dsh available: ${(dsh.stdout || dsh.stderr).trim()}`)
  } else {
    const npx = spawnSync('npx', ['--yes', '@deepseek-ai/dsh', '--version'], { encoding: 'utf8' })
    if (npx.status === 0) ok(`dsh available through npx: ${(npx.stdout || npx.stderr).trim()}`)
    else bad('DeepSeek Harness is unavailable through both dsh and npx @deepseek-ai/dsh')
  }
